...
void main() {
	...
	if (hit_pos != end) {
		...
		float dark = shadow(hit_pos, 0.01f, 1.0f, 8);
		float ao = exp(-march_data.steps / 20.0);
		vec3 diffuse = hit_normal * 0.5 + 0.5;
		FragColor = vec4((phong(hit_pos, hit_normal, ray_direction) + (hit_normal * 0.5 + 0.5)) * dark * ao, 1.0);
	} else {
